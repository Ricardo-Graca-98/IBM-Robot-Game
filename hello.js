...
client.get('statuses/user_timeline', {screen_name: 'DisturbedBoy69', count: '5', include_rts: 'false'} , function(error, tweets, response)
{
    var data = JSON.stringify(tweets, null, 2);
    let profileText = JSON.parse(data);
    for(var i = 0; i < profileText.length;   i++)
    {
        output +=  profileText[i].text;
    }
    console.log(output);
    //fs.writeFileSync('profile.json', JSON.parse(output));
})

//Imports profile.json file.
